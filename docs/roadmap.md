...
**PH4冒頭へ送った積み残し**: 音声判定の価値の実証。実装は動くが、同一台詞・口調違いの
対照実験が**パイプライン経由では差を出していない**(直接比較では25点差)。原因は12秒の
実時間窓が無音と複数発話で均されること。**発話区間(`audio_start_ms/end_ms`)で窓を
切り直してから再実験する**。
...
## 5. PH4のスケジュール(2026-08-03調整)

| | 内容 | 備考 |
|---|---|---|
| PH4.0 | 音声判定の窓を発話区間で切り直し+対照実験 | バックエンドのみ。PH3の積み残し |
| PH4.1 | **SPA化: React + TypeScript + Vite** | 機能等価の書き直し。下記参照 |
| PH4.2 | 認証(Cognito。realtime_voiceから流用) | ログイン画面はSPAの上に作る |
| PH4.3 | 席+認可(3.1〜3.3の順番で。塞いでから鍵をかける) | |
| PH4.4 | 多対多のUI(3.4の優先度で) | フレームワークの恩恵が最大の場所 |
| PH4.5 | STTバックエンド切替(3.5) | |
