...
1. **音声判定の窓を発話区間で切り直し、対照実験をやり直す**(PH3からの積み残し)
2. 認証・認可(3.1の順番で。塞いでから鍵をかける)
3. 席と割り当て(3.2のC方式から)
4. 多対多のUI(3.4の優先度で。怒り順ソートが最上位)
5. STTバックエンド切替(3.5)

なお対照実験の素材について: TTSは「大声で怒鳴る」は演じられるが**「静かに抑えた怒り」は
描き分けられない**(calmとRMS差1dB)。cold系の検証素材が要る場合は人間の録音が必要。
