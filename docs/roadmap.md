...
| **PH4** | **SPA化+認証・認可** | **完了(2026-08-04)**: ①SPA化(PR #11) ②Cognito認証(PR #13) ③SV会話一覧+挙手(PR #14) ④**認可**(PR #15): broadcastの宛先化(応対者=自分の呼の中身のみ・未割り当てライブ呼はメタのみ・call_updatedは全員)、API絞り込み(一覧/詳細/録音/カード403・再処理はSVのみ)、応対者ビュー(カード非表示・再文字起こし不可)、SVアラート音。実Cognitoトークンで境界E2E済み。**未実装の残**: SV閲覧の監査記録(call_views)はPH5へ |
...
プロジェクトをここでアーカイブした。**技術は完走し、電話番号で死んだ。**

### 6.1 死因の記録

1. 開発初期からIaCの流儀どおり `terraform apply` / `destroy` を繰り返した。
   電話番号もその都度取得・解放していた
2. Amazon Connectの電話番号クォータ(L-8F812903、既定5)は、**解放した番号を
   180日間カウントし続ける**(番号の取り回し悪用対策)。8月頭、5枠すべてが
   過去の解放分で埋まり、新規取得が `FAILED` になった。取得失敗もまた枠を食う
3. クォータ引き上げ(5→15)を申請。ARN照会を経て内部審査に回ったが、
   最終的に**事業証明書類・事業内容・用途の3点**を要求された(電気通信系のKYC)。
   開業届で応じる道はあったが、趣味の検証プロジェクトが背負う手続きとしては
   重く、ここで店じまいを決めた
4. 180日カウントが空くのは2027年1月末。「待つ」も実質成立しなかった

教訓: **電話番号はサーバーではなく資産(電話加入権の記憶)。作って壊すIaCの
対象にしてはならない。** 常設方針(コミット 47f8f07)への転換は正しかったが遅かった。

### 6.2 到達点

- KVSのMKV詐称パース(CodecID "A_AAC" で中身は生L16 PCM 8kHz)を自力で突破(PR #1)
- 話者別リアルタイム文字起こし(トラック分離により話者分離が構造的に不要)
- 声のトーン判定: gpt-audioへ音声を渡しJSON指示を**userターンに置く**(0/5→5/5)。
  発話区間で窓を切る。**声は言葉より早い**(テキストが75に達する2発話前に声が72)を実測
- 合成通話フィクスチャ: TTS→電話帯域→KVS互換MKV。実架電なしで全パイプライン検証
- SPA(React+TS+Vite)+Cognito認証(SRP)+認可(broadcastの宛先化・API 403・挙手)
- テスト: バックエンド53本+フロント22本、CI(GitHub Actions)

### 6.3 撤収の記録

- 2026-08-24: `infra/auth`(Cognito、10リソース)と `infra/`(Connect本体、11リソース)を
  destroy。AWS側の残存ゼロを確認(Connectインスタンス0件・KMSエイリアス0件)。課金ゼロ
- クォータ引き上げのサポートケース(178582589400099)は取り下げ
- 全69コミットのシークレット監査クリーン(APIキー・秘密鍵・個人情報の混入なし)

### 6.4 もし掘り起こすなら

- コードは録音リプレイ(`WATCH_CALLS=0` + `/api/replay`)で今でも全機能が動く
- 実PSTNに戻るには: 番号クォータの空き(2027-02以降)を確認してから
  `infra/` を apply し、番号取得は**一発で**行う(失敗リトライは枠を食う)
- 心理分析コアはPSTN非依存。マイク入力や会議音声に載せ替える転生ルートもある
