...
### 電話番号のクォータは「解約後も180日間」消費され続ける → 番号は常設にした

Connectの番号クォータ(Phone numbers per instance、既定5)は、**releaseした番号も
180日間カウントし続ける**(迷惑電話業者の番号ローテーション対策)。apply→検証→
destroyを繰り返す工法では、サイクルごとに枠を1つ(取得失敗のリトライでさらに)
恒久消費し、2026-08-04に番号ゼロなのに「allowed limit exceeded」で取得不能になった。
クォータ引き上げ(L-8F812903、5→15)は申請済み。

**2026-08-04の決定: Connect基盤は常設(destroyしない)。** 番号は使い捨ての設定値
ではなく資産で、待機コストは実測で月$3弱(番号$0.06/日+KMS$1/月。インスタンス
自体は無料、KVS/Lambda/SQSは待機中ほぼゼロ)。取得ガチャと180日クォータからの
解放でお釣りが来る。**「消したら困る」分類はCognito(infra/auth)と番号を含む
Connect基盤の2つになった。** コストを絞る目的のdestroyは今後行わない。
`Phone number not available` の競合時はリトライを連打しない(失敗も枠を食う)。
