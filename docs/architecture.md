...
### 罵倒語は「聞こえたとおりに書け」と指示しないと落ちる

カスハラ検知が目的なのに、**罵倒は定義上「丁寧な文脈から外れた言葉」**なので、
文脈整合を優先すると検知したいものが消える。実測(同一音声を反復):

| prompt | 「ぶっ殺されてんのか」を拾えた回数 |
|---|---|
| なし | 0 / 2 |
| 丁寧な直前文脈のみ | 0 / 2 |
| **逐語指示つき**(罵倒もそのまま書く旨を明示) | **4 / 5** |

ただし**根本原因は音響**にある。電話帯域では4kHz超が最初から存在せず、
このプロジェクトの実測でも 3-4kHz の成分はわずか2.3%しかない。子音の識別に効く
高域が物理的に失われているため、「ぶっころ」と「ひっこ」は本質的に紛らわしい。
**promptで priors を寄せることはできても、失われた情報は復元できない。**

この事実から、怒り判定を罵倒語の正確な聞き取りに依存させる設計は脆い。
声のトーン(韻律・音量・ピッチ)は帯域制限に強く残るので、音声判定は
「あると良い機能」ではなく**核心機能**である。
