...
> **⚰️ このプロジェクトは 2026-08-24 にアーカイブした。**
> 技術検証は完走している——KVSの生MKVパース、話者別リアルタイム文字起こし、
> 声のトーン判定(声は言葉より早い)、SPA+認証認可まで。
> 死因は技術ではなく電話番号だった: Amazon Connectの番号クォータは
> **解放済みの番号を180日間カウントし続け**、IaCの作って壊す開発様式と根本的に相性が悪い。
> クォータ引き上げ審査は事業実体の書類確認まで要求し、個人開発が実PSTNを持ち続ける道は細かった。
> 経緯は [docs/roadmap.md](docs/roadmap.md) の「6. 埋葬」を参照。
> 合成通話フィクスチャ(TTS→電話帯域→KVS互換MKV)により、実架電なしで全パイプラインは今でも動く。
...
- **PH1〜PH4 完了**: Connect基盤IaC → KVSリアルタイム受信 → 話者別文字起こし →
  感情判定(テキスト+声のトーン融合)→ SPA化+Cognito認証+ロール別認可(SV/応対者)
- 実架電E2Eは旧番号で検証済み(2026-07〜08)。AWSリソースは全てdestroy済みで課金ゼロ
- 開発の続きは録音リプレイと合成通話フィクスチャで可能(`WATCH_CALLS=0`)
